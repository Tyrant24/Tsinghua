  ...
  checkPermission() {
    ...
    console.log("走这里没111");
       wx.getStorage({
        key:'tutorid',
        success:function(res){ 
          console.log("走这里没22222");
           memberapi.tutorinfo({id:res.data}, (info) => { 
            AppBase.Tutorinfo = info;
            that.Base.setMyData({tutorinfo:info})
            that.onMyShow(); 
           });
   
          that.Base.setMyData({toturid:res.data})
        },
        fail:(res)=>{
          console.log("走这里没3333");
          if(this.Base.needauth==false){
          wx.reLaunch({
